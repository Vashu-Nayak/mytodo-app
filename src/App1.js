          ...
          {isCompleteScreen===false && allTodos.map((item, index) => {
            if(currentEdit===index){
              <div className="edit_wrapper" key={index}>
                <input placeholder='Updated Title' rows={4} onChange={(e)=>handleUpdateTitle(e.target.value)} value={currentEditedItem.title} />
                <textarea placeholder='Updated Description' onChange={(e)=>handleUpdateDescription(e.target.value)} value={currentEditedItem.description} />
                <button className="primaryBtn" onClick={handleUpdateTodo}>Update</button>
              </div>
            }
            else{
